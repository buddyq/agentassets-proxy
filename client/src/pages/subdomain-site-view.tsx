import { useSiteByHost } from "@/lib/api";
import { useQueryClient } from "@tanstack/react-query";
import { useEffect } from "react";
import SiteView from "./site-view";

interface SubdomainSiteViewProps {
  host: string;
}

export default function SubdomainSiteView({ host }: SubdomainSiteViewProps) {
  const queryClient = useQueryClient();
  const { data: site, isLoading, error } = useSiteByHost(host);
