...
interface GroupRequestEmailData {
  adminEmail: string;
  adminName: string;
  brokerageName: string;
...
export async function sendGroupRequestEmail(data: GroupRequestEmailData): Promise<void> {
  const { 
    adminEmail, 
    adminName, 
    brokerageName, 
  ...
  const logoUrl = brokerageLogo || 'https://atxpocket.nyc3.cdn.digitaloceanspaces.com/static/img/logos/agentassets_logo_white.png';
  ...
  const htmlContent = `
    ...
    <body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: 'Helvetica Neue', Arial, sans-serif;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f3f4f6; padding: 40px 20px;">
        <tr>
          <td align="center">
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 560px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(85, 139, 115, 0.12);">
              
              <!-- Header with Logo -->
              <tr>
                <td style="background: linear-gradient(135deg, #558B73 0%, #3d6b57 100%); padding: 32px 40px; text-align: center;">
                  ${brokerageLogo 
                    ? `<img src="${logoUrl}" alt="${safeBrokerageName}" style="max-height: 48px; max-width: 200px; margin-bottom: 12px;" />`
                    : `<img src="${logoUrl}" alt="AgentAssets" style="max-height: 36px; margin-bottom: 12px;" />`
                  }
                  <div style="color: #ffffff; font-size: 20px; font-weight: 600; letter-spacing: -0.5px;">New Group Request</div>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding: 40px;">
                  <div style="margin-bottom: 24px;">
                    <div style="font-size: 18px; color: #111827; font-weight: 600; margin-bottom: 8px;">Hi ${safeAdminName},</div>
                    <div style="font-size: 15px; color: #6b7280; line-height: 1.6;">
                      A team member has requested a new group for ${safeBrokerageName}.
                    </div>
                  </div>
                  
                  <!-- Request Details Box -->
                  <table width="100%" cellpadding="0" cellspacing="0" style="background: #f9fafb; border-radius: 12px; border-left: 4px solid #558B73; margin-bottom: 24px;">
                    <tr>
                      <td style="padding: 24px;">
                        <div style="margin-bottom: 16px;">
                          <div style="font-size: 11px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Requested By</div>
                          <div style="font-size: 16px; color: #111827; font-weight: 500;">${safeRequesterName}</div>
                          <div style="font-size: 14px; color: #558B73;">
                            <a href="mailto:${safeRequesterEmail}" style="color: #558B73; text-decoration: none;">${safeRequesterEmail}</a>
                          </div>
                        </div>
                        
                        <div style="margin-bottom: ${safeDescription ? '16px' : '0'};">
                          <div style="font-size: 11px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Proposed Group Name</div>
                          <div style="font-size: 18px; color: #111827; font-weight: 600;">${safeGroupName}</div>
                        </div>
                        
                        ${safeDescription ? `
                        <div>
                          <div style="font-size: 11px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Description / Reason</div>
                          <div style="font-size: 14px; color: #374151; line-height: 1.6; background: #ffffff; padding: 12px; border-radius: 8px; border: 1px solid #e5e7eb;">${safeDescription}</div>
                        </div>
                        ` : ''}
                      </td>
                    </tr>
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      <td style="text-align: center;">
                        <a href="https://agentassets.com/brokerage" style="display: inline-block; background: linear-gradient(135deg, #558B73 0%, #3d6b57 100%); color: #ffffff; font-size: 16px; font-weight: 600; padding: 14px 32px; text-decoration: none; border-radius: 10px; box-shadow: 0 4px 14px rgba(85,139,115,0.3);">Go to Brokerage Dashboard</a>
                      </td>
                    </tr>
                  </table>
                  
                  <div style="text-align: center; margin-top: 16px; font-size: 13px; color: #9ca3af;">
                    You can create this group from your brokerage dashboard
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td style="background: #f9fafb; padding: 24px 40px; text-align: center; border-top: 1px solid #e5e7eb;">
                  <div style="margin-bottom: 8px;">
                    <span style="color: #558B73; font-size: 16px; font-weight: 700;">AgentAssets</span>
                  </div>
                  <div style="font-size: 12px; color: #9ca3af;">
                    You received this because you're an admin of ${safeBrokerageName}.<br/>
                    <a href="https://agentassets.com" style="color: #558B73; text-decoration: none;">agentassets.com</a>
                  </div>
                </td>
              </tr>
  ...
  const textContent = `
New Group Request for ${brokerageName}

Hi ${adminName || 'Admin'},

A team member has requested a new group for your brokerage.
...
You can create this group from your brokerage dashboard:
https://agentassets.com/brokerage

---
You received this because you're an admin of ${brokerageName}.
AgentAssets - Beautiful Property Websites for Real Estate Professionals
  `.trim();
  ...
  await transporter.sendMail({
    from: `"AgentAssets" <${process.env.SMTP_FROM || 'no-reply@agentassets.com'}>`,
    to: adminEmail,
    subject: `Group Request: "${groupName}" - ${brokerageName}`,
    text: textContent,
    html: htmlContent,
  });
  console.log(`Group request email sent to ${adminEmail}`);
}
